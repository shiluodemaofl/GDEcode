...
df_first = pd.read_csv('Aquatic_all_first.csv')
df_last = pd.read_csv('Aquatic_all_last.csv')
data = pd.concat([df_first, df_last], ignore_index=True)
...
tabnet_params = dict(
    ...
    gamma=1.3,
    lambda_sparse=1e-4,
    optimizer_params=dict(lr=0.02834295932864115),
...
params_cat = {
    ...
    'iterations': 905,                      # 迭代次数
    'depth': 8,                              # 树的最大深度
    'learning_rate': 0.30203098241011667,      # 学习率
    'l2_leaf_reg': 2.292338872417182,                       # L2 正则化（这里参考 xgb 的 alpha）
    'bagging_temperature': 0.14735770290942846,             # 模拟 subsample 效果
...
for outer_train_idx, outer_val_idx in outer_cv.split(X, y):
    ...
    model_rf_inner = RandomForestClassifier(
        n_estimators=49,           # 随机森林中 100 棵树
        max_features=13,           # 每次分裂时随机选择 15 个特征
        min_samples_leaf=8,        # 每个叶子节点至少包含 1 个样本
        bootstrap=True,            # 启用有放回抽样
        max_samples=0.9761890484008644,           # 每棵树使用 70% 的样本
        max_leaf_nodes=2933,       # 每棵树的最大叶子节点数为 1010
        random_state=2025,# 随机种子设为 2025
        max_depth=28
    )
